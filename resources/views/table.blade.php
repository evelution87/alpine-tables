...
<div x-data="alpinetable(data)">
    ...
    <div class="flex justify-between mt-3 text-gray-700">
        ...
        <template x-if="max_pages > 1">
            <div class="flex border border-gray-200 rounded-md divide-x divide-gray-200 overflow-hidden">
                <div class="p-2 transition" x-on:click="pageDown" :class="Number(page) === 1 ? 'text-gray-200':'cursor-pointer text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </div>
                <div>
                    <select x-model="page" class="border-none focus:ring-0 cursor-pointer text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                        <template x-for="page in max_pages">
                            <option x-text="page" :selected="Number(page) === Number(page)"></option>
                        </template>
                    </select>
                </div>
                <div class="p-2 transition" x-on:click="pageUp" :class="Number(page) === Number(max_pages) ? 'text-gray-200':'cursor-pointer text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
